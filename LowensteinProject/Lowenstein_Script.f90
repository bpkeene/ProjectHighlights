...
!--------------------------------------------------------------------------------------------
  ...
  Try_Again: DO
    IF ((Lowenstein_Ratio .GT. 1.01_DP*Desired_Ratio) .AND. (ntries .LT. 2000)) THEN
      ntries = ntries + 1 
      ...
      CALL lowensteinProtocol(Lowenstein_Mask, Lowenstein_Ratio, Si_Count, Al_Count, &
                              Lowenstein_Si, Lowenstein_Al, Lowenstein_Configuration)
    ELSE IF (ntries .GE. 20000) THEN
      STOP '*** Could not achieve specified Lowenstein Ratio - tried 2000 times ***'
    ELSE
      EXIT Try_Again
    END IF
  END DO Try_Again
